...
void cryptomeetup::airdrop(name from, asset bos) {
    require_auth(_self);    
    /*
...
void cryptomeetup::newland(name from, asset bos) {
    require_auth(_self);
    auto st = _land.available_primary_key();
...
void cryptomeetup::buy_land(name from, extended_asset in, const vector<string>& params) {
    ...
    eosio_assert(in.contract == BOS_CONTRACT, "only true BOS token is allowed");
    eosio_assert(in.quantity.symbol == BOS_SYMBOL, "only true BOS token is allowed...");
    ...
    eosio_assert(in.quantity.amount >= itr->next_price(), "no enough bos");
