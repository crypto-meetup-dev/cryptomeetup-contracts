...
//    TABLE council : public council_info {};
    ...
    TABLE players {
        ...
        uint64_t game_profit;   // 游戏收入(卖land/portal两种情况)以及exceed的 BOS
    ...
    ACTION claim(name from) {
        ...
        if (p.game_profit > 0) {
            action(
                ...
                make_tuple(_self, from, asset(p.game_profit, BOS_SYMBOL),
                    std::string("land_profit & creator_profit & exceed BOS"))
            ).send();    
            p.game_profit = 0;
        }
        if (p.ref_profit > 0) {
    ...
    ACTION newland(name from, asset bos);
    ACTION airdrop(name to, asset bos);
